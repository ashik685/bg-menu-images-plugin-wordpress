...
/**
 * The BG Menu Images Plugin
 *
 * Display image as background to menu item content.
 *
 * @package BG Menu Background Images
 * @subpackage Main
 */
...
class Nav_Menu_Images {
	...
	if ( has_post_thumbnail( $item_id ) ){
			...
			$thumb = wp_get_attachment_image_src( get_post_thumbnail_id($item_id), 'thumbnail_size' );
			$url = $thumb['0'];
			$str = '<div class="menu-thumb " style="background:';
			$str .= "url('";
			$str .= $url;
			$str .= "');";
			$str .= '"><span class="menu-text">'.$content.'</span></div>';
		return $str;
	} else {
	$str = '<div class="menu-thumb "><span class="menu-text">'.$content.'</span></div>';
		return $str;
	}
	}
